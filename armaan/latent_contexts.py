...
arch_name = "LayernormSqueeze1eNeg4lr4eNeg4"
...
""")
    ...
    for seq_ind, (act, tok) in enumerate(zip(activations, toks)):
        for act_ind, (a, t) in enumerate(zip(act, tok)):
            ...
            highlighted_text.append(
                f'<span style="background-color:{background_color};margin-right: {text_spacing}; color:rgb({text_color})">{t.replace(" ", "&nbsp").replace("<bos>","BOS")}</span>'
            )
            if logit_diffs is not None and model_type != "reward_model":
