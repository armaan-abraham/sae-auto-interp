...
def upload_data_dir():
    ...
    s3_client.upload_file(f"{data_dir}.zip", "deep-sae", f"auto-interp/{data_dir.name}.zip")
    import os
...
def download_data_dir(exp_name):
    ...
    import zipfile
    s3_client = boto3.client("s3")
    s3_client.download_file("deep-sae", f"auto-interp/{exp_name}.zip", f"{exp_name}.zip")
